...
struct Matrix[dtype: DType, depth: Int = 1, *, complex: Bool = False](
    ...
    # Fields
    #
    comptime optimal_simd_width = optimal_simd_width[Self.dtype]() // 2 if Self.complex else optimal_simd_width[Self.dtype]()
    ...
    fn __init__(out self, rows: Int, cols: Int, var data: UnsafePointer[Scalar[Self.dtype], MutAnyOrigin]):
        constrained[Self.depth > 0]()
        _assert(rows > 0 and cols > 0, "Rows and cols must be greather than 0")
    ...
    @always_inline
    fn count(self) -> Int:
        return self._rows * self._cols * Self.depth
    ...
    fn real(self: Matrix[Self.dtype, Self.depth, complex=True]) -> Matrix[Self.dtype, Self.depth, complex=False]:
        var result = Matrix[Self.dtype, Self.depth, complex=False](rows=self._rows, cols=self._cols)

        @parameter
        fn take_real_value[width: Int](value: Number[Self.dtype, width, complex=True], row: Int, col: Int, component: Int):
            result._strided_store(Number[Self.dtype, width, complex=False](value.real()), row=row, col=col, component=component)

        self.strided_iterate[take_real_value]()

        return result^

    fn imaginary(self: Matrix[Self.dtype, Self.depth, complex=True]) -> Matrix[Self.dtype, Self.depth, complex=False]:
        var result = Matrix[Self.dtype, Self.depth, complex=False](rows=self._rows, cols=self._cols)

        @parameter
        fn take_imaginary_value[width: Int](value: Number[Self.dtype, width, complex=True], row: Int, col: Int, component: Int):
            result._strided_store(Number[Self.dtype, width, complex=False](value.imaginary()), row=row, col=col, component=component)
    ...
    fn index(self, row: Int, col: Int, component: Int) -> Int:
        return (row * self._cols + col) * Self.depth + component

    @always_inline
    fn flattened_index(self, row: Int, offset: Int) -> Int:
        return row * self._cols * Self.depth + offset
    ...
    ](ref [origin]self) -> MatrixSlice[StridedRange(component, component + 1), Self.dtype, Self.depth, Self.complex, origin]:
        return MatrixSlice[StridedRange(component, component + 1), Self.dtype, Self.depth, Self.complex, origin](self)
    ...
    ](ref [origin]self) -> MatrixSlice[component_range, Self.dtype, Self.depth, Self.complex, origin]:
        return MatrixSlice[component_range, Self.dtype, Self.depth, Self.complex, origin](self)

    @always_inline
    fn strided_slice[
        component_range: StridedRange, mut: Bool, origin: Origin[mut=mut]
    ](ref [origin]self, row_range: StridedRange) raises -> MatrixSlice[component_range, Self.dtype, Self.depth, Self.complex, origin]:
        return self.strided_slice[component_range](row_range=row_range, col_range=StridedRange(self._cols))

    @always_inline
    fn strided_slice[
        component_range: StridedRange, mut: Bool, origin: Origin[mut=mut]
    ](ref [origin]self, *, col_range: StridedRange) raises -> MatrixSlice[component_range, Self.dtype, Self.depth, Self.complex, origin]:
    ...
    fn store_sub_matrix[
        ...
        for matrix_slice_component in range(component_range.count()):
            ...
            fn store_row(matrix_slice_row: Int):
                ...
                vectorize[Self.optimal_simd_width, unroll_factor=unroll_factor](matrix_slice.cols(), store_cols)
    ...
    fn copied_to_component[
        ...
        @parameter
        fn process_row(row: Int):
            fn process_col[width: Int](col: Int) unified {mut result, read self, read row}:
                result._strided_store(self._strided_load[width](row=row, col=col), row=row, col=col, component=component)

            vectorize[Self.optimal_simd_width, unroll_factor=unroll_factor](self._cols, process_col)
        ...
        for component in range(Self.depth):
            ...
            fn calculate_row(row: Int):
                for k in range(shared_dim):
                    ...
                    fn dot_product[width: Int](col: Int) unified {mut result, read self, read rhs, read row, read k}:
    ...
    fn norm(self: Matrix[Self.dtype, Self.depth, complex=True]) -> Matrix[Self.dtype, Self.depth, complex=False]:
        var result = Matrix[Self.dtype, Self.depth, complex=False](rows=self._rows, cols=self._cols)

        @parameter
        fn transform_row(row: Int):
            fn transform_flattened_elements[width: Int](flattened_element: Int) unified {mut result, read self, read row}:
            ...
            vectorize[
                Self.optimal_simd_width,
                unroll_factor=unroll_factor,
            ](self._cols * Self.depth, transform_flattened_elements)
    ...
    fn clamped(self, lower_bound: ScalarNumber[Self.dtype], upper_bound: ScalarNumber[Self.dtype]) -> Self:
        var result = self.copy()
    ...
    fn strided_sum(self, component: Int) -> ScalarNumber[Self.dtype, complex = Self.complex]:
        ...
        for row in range(self._rows):
            ...
            fn process_cols[width: Int](col: Int) unified {mut result, read self, read row, read component}:
                result += self._strided_load[width](row=row, col=col, component=component).reduce_add()

            vectorize[Self.optimal_simd_width, unroll_factor=unroll_factor](self._cols, process_cols)
    ...
    fn strided_min(self: Matrix[Self.dtype, Self.depth], component: Int) -> ScalarNumber[Self.dtype]:
        var result = Scalar[Self.dtype].MAX_FINITE
    ...
    fn strided_max(self: Matrix[Self.dtype, Self.depth], component: Int) -> ScalarNumber[Self.dtype]:
        var result = Scalar[Self.dtype].MIN_FINITE
    ...
    fn strided_for_each_zipped[
        ...
        @parameter
        fn transform_row(row: Int):
            fn transform_cols[width: Int](col: Int) unified {mut self, read other, read row, read component}:
                self._strided_store(
                    transformer[width](
            ...
            vectorize[Self.optimal_simd_width, unroll_factor=unroll_factor](self._cols, transform_cols)
    ...
    fn for_each_zipped[
        ...
        @parameter
        fn transform_row(row: Int):
            fn transform_flattened_elements[width: Int](flattened_element: Int) unified {mut self, read other, read row}:
                var index = self.flattened_index(row=row, offset=flattened_element)
    ...
    fn map_to_range(mut self: Matrix[Self.dtype, Self.depth, complex=False], min: ScalarNumber[Self.dtype], max: ScalarNumber[Self.dtype]):
        @parameter
        for component in range(Self.depth):
            ...
            fn map[width: Int](value: Number[Self.dtype, width, complex=False]) -> Number[Self.dtype, width, complex=False]:
    ...
    fn fourier_transform[*, inverse: Bool = False](self) -> Matrix[fft_dtype, Self.depth, complex=True]:
    ...
    fn padded(self, rows: Int, cols: Int) -> Self:
        ...
        @parameter
        fn copy_row(row: Int):
            memcpy(dest=(result_base_data_ptr + row * new_row_offset), src=(src_data_ptr + row * row_offset), count=row_offset)
    ...
    fn padded_trailing(self, rows: Int, cols: Int) -> Self:
        ...
        @parameter
        fn copy_row(row: Int):
            memcpy(dest=(result_data_ptr + row * new_row_offset), src=(src_data_ptr + row * row_offset), count=row_offset)
    ...
    fn horizontally_stacked(self, other: Self) -> Self:
        ...
        fn process_row(row: Int):
            memcpy(
                dest=(result.unsafe_data_ptr() + row * result_scalar_row_offset),
                src=(self.unsafe_data_ptr() + row * scalar_row_offset),
                count=scalar_row_offset,
            )

            memcpy(
                dest=(right_side_base_data_ptr + row * result_scalar_row_offset),
                src=(other.unsafe_data_ptr() + row * other_scalar_row_offset),
    ...
    fn vertically_stacked(self, other: Self) -> Self:
        ...
        memcpy(dest=(result.unsafe_data_ptr() + self._scalar_count()), src=other.unsafe_data_ptr(), count=other._scalar_count())
    ...
    fn as_type[new_dtype: DType](self) -> Matrix[new_dtype, Self.depth, complex = Self.complex]:
        ...
        if new_dtype == Self.dtype:
            ...
            fn convert_row(row: Int):
                fn convert_flattened_elements[width: Int](flattened_element: Int) unified {mut result, read self, read row}:
                    var index = self.flattened_index(row=row, offset=flattened_element)
                    var value = self._load[width](index).cast[new_dtype]()
                    result._store(value, index=index)

                vectorize[Self.optimal_simd_width, unroll_factor=unroll_factor](self._cols * Self.depth, convert_flattened_elements)

            parallelize[convert_row](self._rows)

            return result^

    fn as_complex[new_dtype: DType = Self.dtype](self) -> Matrix[new_dtype, Self.depth, complex=True]:
        @parameter
        if Self.complex and new_dtype == Self.dtype:
            return rebind[UnsafePointer[Matrix[new_dtype, Self.depth, complex=True], MutAnyOrigin]](UnsafePointer(to=self)).take_pointee()
        elif Self.complex:
            var result = self.as_type[new_dtype]()

            return rebind[UnsafePointer[Matrix[new_dtype, Self.depth, complex=True], MutAnyOrigin]](UnsafePointer(to=result)).take_pointee()
        else:
            var result = Matrix[new_dtype, Self.depth, complex=True](rows=self._rows, cols=self._cols)
        ...
        for row in range(self._rows):
            ...
            for col in range(self._cols):
                ...
                for component in range(Self.depth):
                    ...
                    @parameter
                    if Self.depth > 1:
                        writer.write(", " if component < Self.depth - 1 else "]")
